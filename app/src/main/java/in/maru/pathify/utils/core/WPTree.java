...
public class WPTree {
	...
	public List<String> findPath(String word1, String word2) {
		...
		while (!words.isEmpty() && searchCount > 0) {
			
			WPTreeNode thisNode = words.remove();
			
			List<String> dOne = nw.distanceOne(thisNode.getWord());
			
			for (String child : dOne) {
				if (!visited.contains(child)) {
					WPTreeNode childNode = new WPTreeNode(child, thisNode);
					words.add(childNode);
					visited.add(child);
...
class WPTreeNode {
	...
	public String toString() {
		String pathString = "Word: " + word + ", parent = ";
		if (this.parent == null) {
			pathString += "null.\n";
		} else {
			pathString += this.parent.getWord() + "\n";
		}
		pathString += "[ ";
		for (WPTreeNode thisNode : children) {
			pathString += thisNode.getWord() + ", ";
		}
		pathString += (" ]\n");
		return pathString;
	}
